...
meanSDtoDiamondPlot <- function(dat = NULL,
  ...
  tmpDf <- data.frame(ufs::meanConfInt(mean = means,
                                       sd = sds,
                                       n = ns,
  ...
  if (is.null(colorCol)) {
    plot <- ufs::diamondPlot(tmpDf, yLabels=4, yValues=5, xlab=xlab, ...);
  } else {
    plot <- ufs::diamondPlot(tmpDf, yLabels=4, yValues=5, colorCol=6, xlab=xlab, ...);
  }
